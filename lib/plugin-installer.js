    ...
    constructor(serverUrl = 'http://51.75.25.2:2010') {
        ...
        try {
            ...
            if (cwd.includes(path.sep + 'dist') || cwd.endsWith('dist')) {
                // Extract the dist folder path from cwd using cross-platform approach.
                const distIndex = cwd.lastIndexOf(path.sep + 'dist');
                if (distIndex !== -1) {
                    const distPath = cwd.substring(0, distIndex + path.sep.length + 'dist'.length);
                    defaultAddonsPath = path.join(distPath, 'addons');
                } else if (cwd.endsWith('dist')) {
                    defaultAddonsPath = path.join(cwd, 'addons');
                }
            } else {
                // Not in dist folder: use ./addons in the current directory (same as Windows behavior)
                // This works the same way on Windows, Linux, and Mac
                defaultAddonsPath = path.join(cwd, 'addons');
            }
        } catch (e) {
