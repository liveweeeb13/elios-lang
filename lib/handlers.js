...
module.exports = {
    ...
    handleIsNumeric(args) {
        ...
        // Vérifie si c'est un nombre (entier ou décimal)
        const isNumeric = !isNaN(evaluatedArgs) && !isNaN(parseFloat(evaluatedArgs)) && evaluatedArgs.trim() !== '';

        if (this.debug) {
            console.log(chalk.blue(`[DEBUG IS_NUMERIC] "${chalk.yellow(evaluatedArgs)}" → ${chalk.green(isNumeric)}`));
        }
    ...
    handleIsText(args) {
        ...
        // Vérifie si c'est du texte (non numérique et non vide)
        const isText = evaluatedArgs.trim() !== '' &&
            (isNaN(evaluatedArgs) || isNaN(parseFloat(evaluatedArgs))) &&
            !this.isMathExpression(evaluatedArgs);

        if (this.debug) {
            console.log(chalk.blue(`[DEBUG IS_TEXT] "${chalk.yellow(evaluatedArgs)}" → ${chalk.green(isText)}`));
        }

        return isText;
    },
    // Ajoutez ces fonctions dans lib/handlers.js
    ...
    handleIsPair(args) {
        ...
        const isPair = !isNaN(number) && number % 2 === 0;

        if (this.debug) {
            console.log(chalk.blue(`[DEBUG IS_PAIR] "${chalk.yellow(evaluatedArgs)}" → ${chalk.green(isPair)}`));
        }
    ...
    handleIsUnpair(args) {
        ...
        const isUnpair = !isNaN(number) && number % 2 !== 0;

        if (this.debug) {
            console.log(chalk.blue(`[DEBUG IS_UNPAIR] "${chalk.yellow(evaluatedArgs)}" → ${chalk.green(isUnpair)}`));
        }
